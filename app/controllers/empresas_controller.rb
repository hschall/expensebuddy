class EmpresasController < ApplicationController
  ...
  def index
  @empresas = Empresa.includes(:category)
  ...
  if params[:categoria_filter].present?
    if params[:categoria_filter] == "sin"
      sin_categoria = Category.find_by(name: "Sin categoría")
      @empresas = sin_categoria ? @empresas.where(category_id: sin_categoria.id) : Empresa.none
    else
      categoria = Category.find_by(name: params[:categoria_filter])
  ...
  if params[:empresa_ids].present?
    Empresa.where(id: params[:empresa_ids]).destroy_all
...
def import_from_transactions
  existing_identifiers = Empresa.pluck(:identificador)
  default_category = Category.find_or_create_by!(name: "Sin categoría")

  total_new = 0

  # === Empresas with RFC ===
  new_codes = Transaction
  ...
  new_codes.each do |code|
    description = Transaction.where(company_code: code).pluck(:description).first
    Empresa.create!(
      identificador: code,
  ...
  # Empresas with RFC (company_code present)
  Empresa.where.not(identificador: nil)
         .where.not(identificador: "Sin identificador")
         .where.not(category_id: nil)
         .find_each do |empresa|
    Transaction.where(company_code: empresa.identificador)
  ...
  Empresa.where("identificador LIKE ?", "Sin identificador%")
         .where.not(category_id: nil)
         .find_each do |empresa|
    Transaction.where(company_code: [nil, ""])
  ...
  if @empresa.update(empresa_params)
    respond_to do |format|
      ...
      format.html { redirect_to empresas_path } # Fallback if JS disabled
    end
  else
...
def save_all
  (params[:empresas] || {}).each do |id, attrs|
    empresa = Empresa.find_by(id: id)
    next unless empresa

    empresa.update(category_id: attrs[:category_id])
  end

  # Apply categories to transactions for empresas WITH RFC
  Empresa.where.not(identificador: nil)
         .where.not(identificador: "Sin identificador")
         .where.not(category_id: nil)
         .find_each do |empresa|
  ...
  # Apply categories to transactions for empresas WITHOUT RFC (Sin identificador)
  Empresa.where("identificador LIKE ?", "Sin identificador%")
         .where.not(category_id: nil)
         .find_each do |empresa|
    Transaction.where(company_code: [nil, ""])
